...
data_long_1 <- read.csv(file=paste0("./", "rew-eff-planning-1", "-task-data-long.csv")) %>%
  dplyr::select(-X) %>%
...
# load long format data
data_long_2 <- read.csv(file=paste0("./", "rew-eff-planning-2", "-task-data-long.csv")) %>%
  dplyr::select(-X) %>%
  arrange(subID, taskNo, trialNo) %>%
  mutate(sess = taskNo,
         condition = group,
...
# save a copy
write.csv(data_long_all, file=paste0(task_ver, "-task-data-long.csv"))
...
params90cis <- summary(fit, pars = c("mu_effSens[1]", "mu_effSens[2]",
                                     ...
                                     "effSens_int", "rewSens_int"), probs = c(0.05, 0.95))$summary
print(params90cis)
```
...
data_quest_wide_1 <- read.csv(file=paste0("rew-eff-planning-1", "-self-report-data.csv")) %>%
  dplyr::select(-X, -catchQscorrect)

# load sx data for sample 2
data_quest_wide_2 <- read.csv(file=paste0("rew-eff-planning-2", "-self-report-data.csv")) %>%
  dplyr::select(-X)

# concat
data_quest_wide_all <- rbind(data_quest_wide_1, data_quest_wide_2) %>%
filter(!subID %in% list("5e9f6d2a0f600f0dc07ea1f8", "5fa3f95e37e424055200c834", 
                  "5d862f1717edf1001cce7daf", "5e81d9fafacc788e25ba3435", 
                  "613be4f2386055959ac00bc4")) %>%
  arrange(subID)
...
nPpts <- length(unique(data_quest_wide_all$subID))
...
# save
saveRDS(fit_grm, file = paste0("./stan-fits/", model, "-", task_ver, "-amotivatation-fit.rds"))
...
# get key linking numeric sequential IDs in model outtput back to original prolific identifiers
ID_trans2 <- sx_grm %>%
  dplyr::select(subID, IDsx) %>%
  distinct()

posts_subIDs_grm <- merge(posts_grm, ID_trans2, by="IDsx")
posts_sx_grm <- merge(posts_subIDs_grm, data_quest_wide_all, by = "subID")
...
hte <- posts_sx %>%
  ...
  group_by(subID) %>%
  mutate(planning = ifelse(subID %in% controls, 0, 1),
...
data_long_all_sx <- data_long_all %>%
  filter(subID %in% data_quest_wide_all$subID) %>%
  arrange(subID, sess) %>%
  mutate(ID2 = rep(seq(1, nPpts_sx, 1), each = nTrials_max))

# # check the above has worked as expected
grep(FALSE, unique(data_quest_wide_all$subID)==unique(data_long_all_sx$subID))
