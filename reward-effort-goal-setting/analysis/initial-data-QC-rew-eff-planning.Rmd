---
title: 'modComp study reward-effort task: initial analysis and data QC '
...
Data pre-processing functions (from raw json files) are shown below for full transparency but not actually needed here
...
# here, prolific subIDs have been replaced with dataset-specific uids, for anonyminity
data_long <- read.csv(file=paste0(task_ver, "-task-data-long-anon.csv"))
```

```{r get_ppts_trials}
## get the number of participants and number of trials per participant in the current dataset
nPpts <- length(unique(data_long$uid))  # get number of participants
...
#data_long <- merge(data_long, nTrials_all, by="uid")
...
active_subs <- data_long %>%
  ...
  dplyr::select(uid)
planners <- as.list(unique(active_subs$uid))
```
...
qc_p <- QC_bonus %>%
  ungroup() %>%
  mutate(group = ifelse(uid %in% controls, "control", "planning")) %>%
...
#            group = ifelse(uid %in% controls, "control", "planning")) %>%
#   group_by(uid, condition) %>%
...
```{r overall_stats}
# linear mixed model
lme1 <- lmer(higherEffChosen ~ group * taskNo + trialNo + ( 1 | uid ), data=data_long)
summary(lme1)
anova(lme1)
...
data_aov <- data_long %>%
  group_by(uid, taskNo, group) %>%
  summarise(totalCoins = sum(rewardChosen),
            totalPresses = sum(pressCount)) %>%
  ungroup()

# total reward earned
res.aov <- anova_test(data = data_aov, dv = totalCoins, wid = uid, within = taskNo,
                      between=group)
...
# total presses made
res.aov <- anova_test(data = data_aov, dv = totalPresses, wid = uid, within = taskNo,
                      between=group)
get_anova_table(res.aov)
# pairwise
pwc <- data_aov %>%
...
p <- data_long %>%
  mutate(deltaEffort = (abs(round(trialEffortPropMax1 - trialEffortPropMax2,2))),
         ...
         deltaReward = as.factor(deltaReward)) %>%
...
p <- data_long %>%
  ...
  ungroup() %>%
  ggplot(aes(x=maxPressCount, y=propHigherEff, colour=uid)) +
  geom_point() + 
...
p <- data_long %>%
  ...
  ggplot(aes(x=effortChosenActual, y=pressCount, colour=uid)) +
  geom_point() + 
...
# total failed trials
res.aov <- anova_test(data = data_aov, dv = totalFailed, wid = uid, within = taskNo,
                      between=group)
get_anova_table(res.aov)
# pairwise
pwc <- data_aov %>%
...
p <- data_long %>%
  ...
  ggplot(aes(x=trialStartTimeRel, y=ID, colour=interaction(uid,taskNo))) +
...
data_aov <- data_long %>%
  ...
  ungroup
res.aov <- anova_test(data = data_aov, dv = medianTrialTime, wid = uid, within = c(taskNo), 
                      between=group)
get_anova_table(res.aov)
```
...
# linear mixed model
lme1 <- lmer(choiceRT ~ group * taskNo * trialNo + ( 1 | uid ), data=data_long)
summary(lme1)
anova(lme1)
emmeans(lme1, list(pairwise ~ group*taskNo), adjust = "tukey") #, pbkrtest.limit = 8800
```
...
data_long <- data_long %>%
  mutate(pressRate = (1/(meanPressLag/1000)))
lme1 <- lmer(pressRate ~ group * taskNo * trialNo + ( 1 | uid ), data=data_long)
summary(lme1)
anova(lme1)
...
data_post_ratings_long <- read.csv(file=paste0(task_ver, "-post-task-data-long-anon.csv")) %>%
  dplyr::select(-X)
```
...
r1 <- data_post_ratings_long %>%
  ...
  mutate(condition = ifelse((uid %in% controls), "control", "planning"),
...
data_post_long_1 <- data_post_ratings_long %>%
  mutate(condition = ifelse((uid %in% controls), "control", "planning"),
         postTask = ifelse((taskNo==0), "post task 1", "post task 2")) %>%
  filter(questionNo==1)
lme1 <- lmer(answer ~ condition * postTask * questionStage + ( 1 | uid ), data=data_post_long_1)
anova(lme1)
...
data_post_long_2 <- data_post_ratings_long %>%
  mutate(condition = ifelse((uid %in% controls), "control", "planning"),
         postTask = ifelse((taskNo==0), "post task 1", "post task 2")) %>%
  filter(questionNo==2)
lme2 <- lmer(answer ~ condition * postTask * questionStage + ( 1 | uid ), data=data_post_long_2)
anova(lme2)
...
data_post_long_3 <- data_post_ratings_long %>%
  mutate(condition = ifelse((uid %in% controls), "control", "planning"),
         postTask = ifelse((taskNo==0), "post task 1", "post task 2")) %>%
  filter(questionNo==3)
lme3 <- lmer(answer ~ condition * postTask * questionStage + ( 1 | uid ), data=data_post_long_3)
anova(lme3)
...
# data_quest_wide <- pivot_wider(data_quest_long, id_cols = uid, names_from = quest, values_from = answer) %>%
...
p9 <- data_quest_wide %>%
  mutate(condition = ifelse((uid %in% controls), "control", "planning"),
         ...
         tx_selfGuided = ifelse(grepl("self-guided", demogs_tx), "yes", "no"),
         tx_other = ifelse(grepl("other", demogs_tx), "yes", "no")) %>%
  dplyr::select(uid, condition, contains("tx"), -demogs_tx) %>%
  melt(id.vars=c("uid", "condition")) %>%
...
# tmp <- data_quest_wide %>%
#   dplyr::select(uid, catchQsCorrect) %>%
...
# # OR, load data
misc_data_wide <- read.csv(file=paste0(task_ver, "-misc-data-anon.csv")) %>%
  dplyr::select(-X)
...
p1 <- misc_data_wide %>%
  ...
  ggplot(aes(x=variable, y=value, group=uid, fill=uid, colour=uid)) +
  geom_point() +
...
misc_data_wide <- merge(misc_data_wide, tmp, by = "uid") 
...
p3 <- misc_data_wide %>%
  ...
  pivot_wider(id_cols=c("uid","blockNo","condition"), names_from = "variable", 
              values_from = "value") %>%
...
data_aov2 <- misc_data_wide %>%
    mutate(group = ifelse((uid %in% controls), "control", "planning")) %>%
  dplyr::select(uid, group, block0answer, block1answer, block2answer, block3answer) %>%
  melt(id.vars=c("uid", "group")) %>%
  mutate(variable = recode(variable, 
                           'block0answer'=0, "block1answer"=1, "block2answer"=2, "block3answer"=3))

data_lm <- data_aov2 %>%
  mutate(block = as.factor(variable))
lme1 <- lmer(value ~ group*block + ( 1 | uid ), data=data_lm)
anova(lme1)
