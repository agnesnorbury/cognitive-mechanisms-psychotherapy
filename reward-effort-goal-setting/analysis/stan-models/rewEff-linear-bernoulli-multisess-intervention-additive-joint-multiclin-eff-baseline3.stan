...
parameters {
  // group-level correlation matrices across tasks (cholesky factors for faster computation)
  ...
  // vector[nTraits] mu_thetas; 
  // // sds for each trait
  // vector<lower=0>[nTraits] sigma_thetas;   // sigma must be positive for normal dist
  
  // individual-level parameters for self-report traits
  real theta_a[nPpts];
  real theta_n[nPpts];
...
transformed parameters {
  ...
  for ( p in 1:nPpts ) {
    // // latent trait params
    // theta_a[p] = (mu_thetas[1] + sigma_thetas[1] * theta_a_raw[p]);    
    // theta_n[p] = (mu_thetas[2] + sigma_thetas[2] * theta_n_raw[p]);   
    
    // task time 1 params
    rewSens[p,1] = -1 + Phi_approx(mu_rewSens[1] + rewSens_tilde[1,p])*4;
    effSens[p,1] = -1 + Phi_approx(mu_effSens[1] + effSens_tilde[1,p] + beta_b1*theta_a[p] + beta_b2*theta_n[p])*10;
   
    // task time 2 params
    if ( condition[p] == 1) {
      rewSens[p,2] = -1 + Phi_approx(mu_rewSens[2] + rewSens_tilde[2,p] + rewSens_int)*4;
      effSens[p,2] = -1 + Phi_approx(mu_effSens[2] + effSens_tilde[2,p] + effSens_int + beta_int1*theta_a[p] + beta_int2*theta_n[p])*10;  //+ beta_bint*effSens[p,1]
    } else {
...
model {
  ...
  // theta_a_raw ~ normal(0,1);          // individual values
  // theta_n_raw ~ normal(0,1);          // individual values
...
generated quantities {
  ...
  corr_matrix[nTimes]  R_rewSens;
  corr_matrix[nTimes]  R_effSens;
