...
parameters {
  ...
  // beta weights for relating self-report to intervention effects
  real beta_int;
  real b1;
  //real b2;
  //real b3;
...
transformed parameters {
  ...
  for ( p in 1:nPpts ) {
    ...
    theta_internal_pos[p,1] = mu_internal_theta_pos[1] + pars_tilde_pos[1,p] + b1*theta[p];
    theta_global_pos[p,1]   = mu_global_theta_pos[1]   + pars_tilde_pos[2,p];
    // positive events at time 2
    if ( condition[p] == 1 ) {
      // for active intervention participants
      theta_internal_pos[p,2] = mu_internal_theta_pos[2] + pars_tilde_pos[3,p] + theta_int_internal_pos + beta_int*theta[p];
