---
title: 'modComp study causal attribution task: cognitive restructuring intervention data '
...
The below functions provided for transparency but not needed here (since the raw data has already been reformatted from json and concatenated into a csv file)
...
data_long_choice <- read.csv(file=paste0(task_ver, "-choice-task-data-long-anon.csv"))
...
active_subs <- data_long_choice %>%
  filter(condition=="psychoed") %>%
   dplyr::select(uid)
actives <- as.list(unique(active_subs$uid))
```
...
rp1 <- data_long_choice %>%
  ...
  dplyr::select(uid, valence, taskNo, condition, internalChosen) %>%
  melt(id.vars=c("uid", "valence", "taskNo", "condition")) %>%
  mutate(taskNo = as.factor(taskNo)) %>%
  group_by(uid, valence, taskNo, condition) %>%
  ...
  geom_line(aes(group=uid, colour=condition), alpha = .4) +
...
rp2 <- data_long_choice %>%
  mutate(condition = ifelse(condition=="psychoed", "psychoeducation", "control")) %>%
  dplyr::select(uid, valence, taskNo, condition, globalChosen) %>%
  melt(id.vars=c("uid", "valence", "taskNo", "condition")) %>%
  mutate(taskNo = as.factor(taskNo)) %>%
  group_by(uid, valence, taskNo, condition) %>%
  summarise(mean_internal = mean(value)) %>%
...
```{r basic_stats}
# linear mixed models
lme1 <- lmer(internalChosen ~ valence * condition * taskNo +  ( 1 | uid ),
...
lme1 <- lmer(globalChosen ~ valence * condition * taskNo +  ( 1 | uid ),
            data=data_long_choice)
summary(lme1)
anova(lme1)
emmeans(lme1, list(pairwise ~ condition*valence*taskNo), adjust = "tukey") #, pbkrtest.limit = 6400
...
```{r choice_rt}
p <- data_long_choice %>%
  dplyr::select(uid, valence, trialNo, taskNo, condition, rt) %>%
  group_by(uid, taskNo, valence) %>%
...
# linear mixed model
lme1 <- lmer(rt ~ valence * condition * taskNo + (1 | uid:taskNo), data=data_long_choice)
summary(lme1)
anova(lme1)
```
...
data_aov <- QC_bonus_ch %>%
  dplyr::select(uid, condition, intTime) %>%
  group_by(uid, condition) %>%
  summarise(mean_int_time = mean(intTime)) %>%
  ungroup
res.aov <- anova_test(data = data_aov, dv = mean_int_time, wid = uid, between=condition)
...
# data_quest_wide <- pivot_wider(data_quest_long, id_cols = uid, names_from = quest,
#                                values_from = answer) %>%
#   dplyr::select(-P0_Q0, -study_feedback) %>%
#   mutate(demogs_age = replace(demogs_age, uid=="5ed7a0239e6b4a23ec6eb236", 21),
#          demogs_gender = replace(demogs_gender, uid=="5ed7a0239e6b4a23ec6eb236", "man")) %>%
...
```{r plot_demogs}
p1 <- data_quest_wide %>%
  filter(!uid=="subCA1_45") %>%  # rm one participant missing this data
  mutate(group = ifelse((uid %in% controls), "control", "psychoeducation")) %>%
...
p9 <- data_quest_wide %>%
  filter(!uid=="subCA1_45") %>%
  mutate(condition = ifelse((uid %in% controls), "control", "psychoeducation"),
         tx_talkingTher = ifelse(grepl("talking therapy", demogs_tx), "yes", "no"),
         tx_meds = ifelse(grepl("medication", demogs_tx), "yes", "no"),
         tx_selfGuided = ifelse(grepl("self-guided", demogs_tx), "yes", "no"),
         tx_other = ifelse(grepl("other", demogs_tx), "yes", "no")) %>%
...
p10 <- data_quest_wide %>%
  ...
  dplyr::select(uid, contains("dis"), -demogs_disability) %>%
  melt(id.vars=c("uid")) %>%
  ggplot(aes(x=variable, fill=value, group=interaction(variable, value))) +
  geom_bar(alpha=.6, position = "dodge") +
...
# # catch questions
# tmp <- data_quest_wide %>%
#   dplyr::select(contains("catch_"), uid) %>%
...
# data_quest_wide <- merge(data_quest_wide, tmp, by = "uid")
#
# # PHQ9
